...
count_neighbors :: #force_inline proc(w: ^World, x: i32, y: i32) -> u8 {
	...
	up		:= (y - 1) %% w.height
	down	:= (y + 1) %% w.height
	fmt.println("%% remainder (floored):", (x - 1) %% w.width)
	fmt.println("% modulo (truncated):", (x-1)%w.width)
...
main :: proc() {
	...
	cell := Cell {
		...
		height = f32(window.height) / f32(world.height),
	}
	...
	for !rl.WindowShouldClose() {
		...
		// The background gets cleared to a hight contrast color, so it's easy
		// to se if there was any pixed missed
		rl.BeginDrawing()
