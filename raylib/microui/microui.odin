...
main :: proc() {
	rl.InitWindow(state.screen_width, state.screen_height, "micro-raylib-odin")
...
render :: proc "contextless" (ctx: ^mu.Context) {
	render_texture :: proc "contextless" (renderer: rl.RenderTexture2D, 
		dst: ^rl.Rectangle, src: mu.Rect, color: rl.Color) {
		dst.width = f32(src.w)
	...
	if mu.window(ctx, "Demo Window", {40, 40, 300, 450}, opts) {
		...
		if .ACTIVE in mu.header(ctx, "Background Colour", {.EXPANDED}) {
			...
			mu.draw_control_text(
				ctx, 
				fmt.tprintf("#%02x%02x%02x", state.bg.r, state.bg.g, state.bg.b), r, .TEXT, {.ALIGN_CENTER}
			)
		}
	}
